...
import { MovementType, ProductSelection, StockMovement } from "../../services/stockMovmentService";

// Simple department type for our UI
...
export default function HistoryScreen() {
  ...
  // Extract unique departments from movements
  const extractDepartmentsFromMovements = useCallback((movements: StockMovement[]) => {
    const departmentMap = new Map<string, DepartmentConfig>();
    
    // Always include "All" option
    departmentMap.set('all', { 
      key: 'all', 
      label: 'All', 
      icon: '🏢', 
      color: '#6b7280', 
    ...
    movements.forEach((movement) => {
      ...
      if (typeof movement.department === 'string') {
        departmentKey = movement.department;
        departmentName = movement.department;
      } else if (typeof movement.department === 'object' && movement.department !== null) {
        if ('id' in movement.department) {
          departmentKey = (movement.department as any).id;
          departmentName = (movement.department as any).name || departmentKey;
        } else if ('name' in movement.department) {
          departmentKey = (movement.department as any).name;
          departmentName = (movement.department as any).name;
        }
      }

      if (departmentKey && !departmentMap.has(departmentKey)) {
        // Generate a consistent color based on the department key
        const colors = ['#6366f1', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#06b6d4', '#84cc16', '#f97316'];
        const colorIndex = departmentMap.size % colors.length;
  ...
  // Update departments when movements change
  useEffect(() => {
    if (movements.length > 0) {
      const extractedDepartments = extractDepartmentsFromMovements(movements);
      setDepartments(extractedDepartments.length > 1 ? extractedDepartments : defaultDepartments);
    } else {
      setDepartments(defaultDepartments);
    }
  }, [movements, extractDepartmentsFromMovements]);
  ...
  const filteredMovements = movements.filter((movement: StockMovement) => {
    ...
    const matchesSearch = debouncedSearchQuery === '' || 
      ...
      (movement.department && getDepartmentDisplay(movement).toLowerCase().includes(debouncedSearchQuery.toLowerCase()));
  ...
  const getDepartmentColor = (movement: StockMovement): string => {
    const departmentKey = getDepartmentKey(movement);
    if (!departmentKey) return '#6b7280';
    
    const dept = departments.find(d => d.key === departmentKey);
    return dept?.color || '#6366f1';
  };

  const getDepartmentKey = (movement: StockMovement): string => {
    if (!movement.department) return '';
    ...
    if (typeof movement.department === 'object' && movement.department !== null) {
      if ('id' in movement.department) {
        return (movement.department as any).id;
      }
      if ('name' in movement.department) {
  ...
  return (
    <View style={styles.container}>
      ...
      {apiError && (
        <View style={styles.quotaErrorContainer}>
          ...
          <View style={styles.errorActions}>
            ...
            <TouchableOpacity onPress={onRefresh} style={styles.retrySmallButton}>
      ...
      <View style={styles.filtersContainer}>
        ...
        {/* Department Filter - Only show if we have departments */}
