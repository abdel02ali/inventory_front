...
const retryWithBackoff = async (fn: () => Promise<any>, retries = 3, delay = 1000) => {
  try {
    ...
    if (retries > 0 && (error.code === 8 || error.details?.includes('Quota exceeded'))) {
...
export default function HistoryScreen() {
  ...
  // Refetch departments when screen comes into focus
  useFocusEffect(
    useCallback(() => {
      console.log('🔄 Refetching departments on focus');
      fetchDepartments();
    }, [])
  );
  ...
  const { 
    ...
    page: 1,
    limit: 30 // Reduced from 50 to 30 to save quota
  });

  // Optimized refetch function with error handling and retry logic
  const handleRefetch = async () => {
    if (isRefetching) return; // Prevent multiple simultaneous requests
    ...
    try {
      ...
      await retryWithBackoff(async () => {
        await refetch();
        // Also refetch departments when we refetch movements
  ...
  // Refetch data only when filters change (not on focus) and only for meaningful changes
  useEffect(() => {
    const shouldRefetch = selectedType !== 'all' || selectedDepartment !== 'all';
    
    if (shouldRefetch && !loading) {
      console.log('🔄 Refetching due to filter change');
      handleRefetch();
    }
  }, [selectedType, selectedDepartment]);

  // Show error alert if there's an error from the hook
  useEffect(() => {
    if (error && !apiError) {
      setApiError(error);
    }
  ...
  const getDepartmentDisplay = (movement: StockMovement): string => {
    ...
    // Final fallback - show the raw department data
    return `🏢 ${departmentId || 'Unknown'}`;
  };

  // Fixed renderProductItem function - remove key prop
  ...
  // Function to detect and add missing departments from movements
  const addMissingDepartmentsFromMovements = useCallback(() => {
    if (!movements.length) return;

    const currentDepartmentKeys = new Set(departments.map(d => d.key));
    const missingDepartments: DepartmentConfig[] = [];

    movements.forEach((movement: StockMovement) => {
      if (!movement.department) return;

      const departmentId = getDepartmentIdFromMovement(movement);
      if (!departmentId || departmentId === 'all' || currentDepartmentKeys.has(departmentId as DepartmentFilter)) {
        return;
      }

      // Check if this department is already in our missing list
      if (!missingDepartments.find(d => d.key === departmentId)) {
        missingDepartments.push({
          key: departmentId as DepartmentFilter,
          label: typeof movement.department === 'object' && movement.department !== null && 'name' in movement.department 
            ? (movement.department as any).name 
            : departmentId,
          icon: '🏢',
          color: '#6366f1',
          activeColor: '#6366f1'
    ...
    if (missingDepartments.length > 0) {
      console.log('➕ Adding missing departments from movements:', missingDepartments);
      setDepartments(prev => [...prev, ...missingDepartments]);
    }
  }, [movements, departments]);

  // Add missing departments when movements change
  useEffect(() => {
    addMissingDepartmentsFromMovements();
  }, [movements, addMissingDepartmentsFromMovements]);
    ...
    <View style={styles.container}>
      ...
      {apiError && (
        <View style={styles.quotaErrorContainer}>
          ...
          <Text style={styles.quotaErrorText}>{apiError}</Text>
          <View style={styles.errorActions}>
            <TouchableOpacity onPress={clearError} style={styles.dismissButton}>
              <Text style={styles.dismissButtonText}>Dismiss</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={handleRefetch} style={styles.retrySmallButton}>
      ...
      {filteredMovements.length === 0 ? (
        <View style={styles.emptyState}>
          ...
          <TouchableOpacity style={styles.emptyStateButton} onPress={handleRefetch}>
        ...
        <SectionList
          ...
          renderItem={({ item }: { item: StockMovement }) => (
            ...
            >
              ...
              {/* Products List - Fixed key placement */}
...
// ... keep your existing getStyles function exactly as it is ...
