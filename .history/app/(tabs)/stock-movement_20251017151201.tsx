...
useEffect(() => {
  if (productDropdownVisible) {
    ...
    (async () => {
      ...
      console.log('✅ Products refreshed, new availableProducts:', products?.length, 'products');
      console.log('📋 Products data:', products?.map(p => ({ id: p.id, name: p.name, quantity: p.quantity })));
    })();
  }
}, [productDropdownVisible]); // <-- CHANGE THIS to productDropdownVisible
...
// Product dropdown functions - fix the refresh logic
const openProductDropdown = async (index: number) => {
  console.log("📦 Opening product dropdown for index:", index);
  
  try {
    await refreshProducts(); // Make sure this is uncommented and working
  } catch (error) {
