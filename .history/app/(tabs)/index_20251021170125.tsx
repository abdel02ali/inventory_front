...
export default function Dashboard() {
  ...
  const calculateChange = (current: number, previous: number): string => {
    if (previous === 0) {
      // If previous was 0 and current is > 0, it's +100%
    ...
    // Format the change with + or - sign and no decimal places
  ...
  try {
    ...
    // CORRECTION : Adapter les données de l'API
    const apiData = statsResponse.data;
    const adaptedStats = {
      totalProducts: apiData.totalProducts || 0,
      outOfStock: apiData.outOfStock || apiData.lowStockItems || 0,
