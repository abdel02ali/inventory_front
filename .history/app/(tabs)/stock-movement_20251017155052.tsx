...
// In your stock-movement.tsx, replace the useEffect and getProductStock function:

  // 🔄 Auto-refresh products when dropdown opens - FIXED VERSION
  useEffect(() => {
    const refreshProductsForDropdown = async () => {
    ...
    refreshProductsForDropdown();
  }, [productDropdownVisible, refreshProducts]);

  // Helper function to get actual stock (simplified since we normalized the data)
  const getProductStock = (product: any) => {
    // Now we can just use product.quantity since we normalized the data
...
const openProductDropdown = async (index: number) => {
  ...
  // Refresh before showing modal (not inside useEffect!)
