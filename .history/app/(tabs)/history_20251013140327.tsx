...
import { Department, MovementType, StockMovement } from '../../services/stockMovmentService';
...
export default function HistoryScreen() {
  ...
  const departments: { key: DepartmentFilter; label: string; icon: string; color: string }[] = [
    ...
    { key: 'magasin', label: 'Management', icon: '👔', color: '#0ea5e9' },
  ];
  ...
  const filteredMovements = movements.filter(movement => {
    ...
    const matchesSearch = searchQuery === '' || 
      movement.products.some(product => 
  ...
  const groupedMovements = filteredMovements.reduce((acc: { [key: string]: StockMovement[] }, movement) => {
  ...
  // Statistics
  const totalStockIn = filteredMovements.filter(m => m.type === 'stock_in').length;
  const totalDistributions = filteredMovements.filter(m => m.type === 'distribution').length;
            ...
            <View style={styles.historyItem}>
                ...
                {item.products.map((product, index) => (
                  <View key={`${product.productId}-${index}`} style={styles.productItem}>
                    ...
                    {product.price && product.price > 0 && (
                      <Text style={styles.productPrice}>{product.price.toFixed(2)} MAD</Text>
                    )}
                  </View>
                ))}
              </View>
              ...
              {item.type === 'stock_in' && item.totalValue && item.totalValue > 0 && (
                <View style={styles.totalValueContainer}>
                  ...
                  <Text style={styles.totalValue}>{item.totalValue.toFixed(2)} MAD</Text>
                </View>
              )}
