...
export default function ProductsScreen() {
  ...
  const handleStockUpdate = async (product: Product, newQuantity: number) => {
    try {
      console.log('🔄 Updating stock for:', product.name, 'New quantity:', newQuantity);
      ...
      // Trigger notifications based on new quantity
      const lowStockThreshold = product.lowStockThreshold || 10;
      
      if (newQuantity === 0) {
        await scheduleOutOfStockAlert(product.name);
        console.log('🚨 Out of stock notification sent for:', product.name);
      } else if (newQuantity <= lowStockThreshold) {
        await scheduleLowStockAlert(product.name, newQuantity);
        console.log('⚠️ Low stock notification sent for:', product.name, 'Current:', newQuantity);
      }
      
      // Show success message
      Alert.alert(
        'Stock Updated', 
        `${product.name} stock updated to ${newQuantity}`,
        [{ text: 'OK' }]
      );
...
const fetchProducts = async (isRefresh = false) => {
  ...
  const renderProductItem: ListRenderItem<Product> = ({ item }) => {
    ...
    const quickAdjustStock = (adjustment: number) => {
      const newQuantity = Math.max(0, displayQuantity + adjustment);
      handleStockUpdate(item, newQuantity);
    };
      ...
      >
        ...
        <View style={styles.productInfo}>
          ...
          <View style={styles.quickActions}>
            <Text style={styles.quickActionsLabel}>Quick Adjust:</Text>
            <View style={styles.quickActionButtons}>
              <TouchableOpacity 
                style={[styles.quickActionButton, styles.decreaseButton]}
                onPress={() => quickAdjustStock(-1)}
              >
                <Text style={styles.quickActionText}>-1</Text>
              </TouchableOpacity>
              <TouchableOpacity 
                style={[styles.quickActionButton, styles.increaseButton]}
                onPress={() => quickAdjustStock(1)}
              >
                <Text style={styles.quickActionText}>+1</Text>
              </TouchableOpacity>
              <TouchableOpacity 
                style={[styles.quickActionButton, styles.restockButton]}
                onPress={() => quickAdjustStock(10)}
              >
...
const getStyles = (isDarkMode: boolean) => StyleSheet.create({
  ...
  increaseButton: {
    backgroundColor: isDarkMode ? '#14532d' : '#bbf7d0',
  },
  restockButton: {
