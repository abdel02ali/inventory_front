...
export default function StockMovementScreen() {
  ...
  // 🔄 Auto-refresh products when dropdown opens - FIXED DEPENDENCY
  useEffect(() => {
    console.log('🔄 useEffect triggered, productDropdownVisible:', productDropdownVisible);
    if (productDropdownVisible) {
      console.log('🔄 Auto-refreshing products for dropdown');
      console.log('📊 Current availableProducts before refresh:', availableProducts.length, 'products');
      
      (async () => {
        try {
          await refreshProducts();
          console.log('✅ Products refreshed, new availableProducts:', availableProducts.length, 'products');
          console.log('📋 Products data:', availableProducts.map(p => ({ id: p.id, name: p.name, quantity: p.quantity })));
        } catch (error) {
  ...
  const openProductDropdown = async (index: number) => {
    ...
    console.log("📊 Current availableProducts on open:", availableProducts.length, 'products');
    
    try {
      await refreshProducts();
      console.log("✅ After refresh - availableProducts:", availableProducts.length, 'products');
    } catch (error) {
      console.log("❌ Error refreshing products:", error);
    }
  ...
  const ProductDropdownModal = () => {
    console.log("🎪 ProductDropdownModal rendering, visible:", productDropdownVisible);
    console.log("📋 filteredProducts for dropdown:", filteredProducts.length, 'products');
              ...
              <View style={styles.dropdownContainer}>
                ...
                <FlatList
                  ...
                  renderItem={({ item, index }) => {
                    ...
                    console.log(`📦 Rendering product ${index}:`, item.name, 'stock:', currentStock);
                  ...
                  ListEmptyComponent={
                    <View style={styles.emptyDropdown}>
                      ...
                      <Text style={styles.availableProductsText}>
                        Available products in context: {availableProducts.length}
                      </Text>
                      <TouchableOpacity 
