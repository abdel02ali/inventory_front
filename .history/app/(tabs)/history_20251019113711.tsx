...
// Define what the useStockMovements hook expects for Department
// This should match the Department type from stockMovmentService
type StockMovementDepartment = 'pastry' | 'bakery' | 'cleaning' | 'magazin' | string;
...
export default function HistoryScreen() {
    ...
    const fetchDepartments = async () => {
      try {
        ...
        if (Array.isArray(serverDepartments)) {
          serverDepartments.forEach((dept: DepartmentType) => {
            if (dept && dept.id) {
              transformedDepartments.push({
                key: dept.id, // Use the department ID as the key
  ...
  // Convert our department string to the type expected by useStockMovements
  const getDepartmentForHook = (department: string): StockMovementDepartment | undefined => {
    if (department === 'all') return undefined;
    return department as StockMovementDepartment;
  };
  ...
  const { 
    ...
    department: getDepartmentForHook(selectedDepartment),
    page: 1,
    limit: 50
  });
  ...
  const getDepartmentDisplay = (department: string): string => {
    ...
    // Fallback: return the department key if no match found
    return `${department}`;
  };
...
// ... keep the same getStyles function ...
