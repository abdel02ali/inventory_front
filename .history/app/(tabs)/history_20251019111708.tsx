...
export default function HistoryScreen() {
    ...
    const fetchDepartments = async () => {
      try {
        ...
        if (Array.isArray(serverDepartments)) {
          serverDepartments.forEach((dept: any) => {
            if (dept && dept.id) {
              transformedDepartments.push({
                key: dept.id as Department, // Use the department ID as the key
        ...
        } else if (typeof serverDepartments === 'object' && serverDepartments !== null) {
          // Handle object format (key-value pairs)
          Object.entries(serverDepartments).forEach(([key, dept]: [string, any]) => {
            if (dept && dept.name) {
              transformedDepartments.push({
  ...
  const getDepartmentDisplay = (department: Department): string => {
    const dept = departments.find(d => d.key === department);
    if (dept) {
