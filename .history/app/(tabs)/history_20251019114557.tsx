...
export default function HistoryScreen() {
  ...
  const getDepartmentColor = (department?: StockMovementDepartment) => {
    if (!department) return '#6b7280';
    const dept = departments.find(d => d.key === department);
    return dept?.color || '#6366f1';
  };

  const getDepartmentDisplay = (department: StockMovementDepartment): string => {
    ...
    if (dept) {
      const displayText = `${dept.icon} ${dept.label}`;
      console.log('Display text:', displayText);
      return displayText;
    }
    // Fallback: return the department key if no match found
    console.log('Department not found, using fallback:', department);
    return `${department}`;
  };
