...
// 🔄 Fix the useEffect to refresh products when dropdown opens
useEffect(() => {
  if (productDropdownVisible) {
    console.log('🔄 Auto-refreshing products for dropdown');
    refreshProducts();
  }
}, [productDropdownVisible]); // <-- Change dependency to productDropdownVisible

// Product dropdown functions - uncomment the refreshProducts call
const openProductDropdown = async (index: number) => {
  console.log("📦 Opening product dropdown for index:", index);
  
  // Uncomment this line to refresh products before showing modal
