// app/stock-movement.tsx (updated to use exact data types)
...
export default function StockMovementScreen() {
  ...
  const [selectedDepartment, setSelectedDepartment] = useState<StockMovementDepartment | ''>('');
  ...
  useEffect(() => {
    const loadDepartments = async () => {
      try {
        ...
        if (depts.length > 0) {
          setSelectedDepartment(depts[0].id as StockMovementDepartment);
        }
      } catch (error) {
  ...
  const selectDepartment = (departmentId: string) => {
    setSelectedDepartment(departmentId as StockMovementDepartment);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const productsData: StockProductSelection[] = selectedProducts.map(product => ({
        ...
        unit: product.unit
        // unitPrice is optional, you can add it if needed
      }));
          ...
          [
            {
              ...
              onPress: () => {
                ...
                if (departments.length > 0) {
                  setSelectedDepartment(departments[0].id as StockMovementDepartment);
                }
                router.back();
              }
            }
