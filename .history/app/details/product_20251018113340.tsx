...
export default function ProductDetailScreen() {
  ...
  const { id, name, quantity: q, unit, categories, category, description } = useLocalSearchParams();
  ...
  const getCategoriesFromParams = () => {
    // Try categories first (array), then category (string), then default
    try {
      const categoriesParam = Array.isArray(categories) ? categories[0] : categories;
      if (categoriesParam) {
        const parsed = JSON.parse(categoriesParam);
        return Array.isArray(parsed) ? parsed : [parsed];
      }
    } catch (e) {
      // If not JSON, try single category
  ...
  const [editedCategories, setEditedCategories] = useState<string[]>(getCategoriesFromParams());
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const updateData = {
        ...
        primaryCategory: editedCategories[0],
  ...
  const handleCancelEdit = () => {
    ...
    setEditedCategories(getCategoriesFromParams());
