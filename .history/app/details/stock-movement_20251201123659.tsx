...
export default function StockMovementScreen() {
  ...
  const handleSubmit = async () => {
    ...
    const invalidProducts = selectedProducts.filter(product => {
      if (!product.productId || !product.quantity) return true;
      
      // Convert quantity to number for validation
      const quantityNum = parseFloat(product.quantity.replace(',', '.'));
    ...
    try {
      ...
      const productsData: StockProductSelection[] = selectedProducts.map(product => ({
        ...
        quantity: parseFloat(product.quantity.replace(',', '.')), // Convert to float
        unit: product.unit
      }));
                      ...
                      <View style={styles.productRow}>
                        ...
                        <View style={[styles.inputGroup, styles.quantityInput]}>
                          <Text style={styles.inputLabel}>
                            Quantity * 
                            {product.unit && (
                              <Text style={styles.unitTypeHint}>
                                {isDecimalUnit ? ' (use . or , for decimals)' : ' (whole numbers only)'}
                              </Text>
                            )}
                          </Text>
                          <TextInput
