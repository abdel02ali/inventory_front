  ...
  <View>
    ...
    {(duplicateProducts.length > 0 || invalidQuantities.length > 0 || (hasRemoveActions && !isRecipientValid)) && (
      <View style={[styles.validationSummary, colorScheme === "dark" && styles.validationSummaryDark]}>
        ...
        {/* Combine all validation items into one array */}
        {[
          ...duplicateProducts.map((id) => (
            `• Remove duplicate: ${getProductName(id!)}`
          )),
          ...invalidQuantities,
          ...(hasRemoveActions && !isRecipientValid ? ['• Select who is taking the products from stock'] : [])
        ].map((message, index) => (
          <Text 
            key={index}
            style={[styles.validationItem, colorScheme === "dark" && styles.validationItemDark]}
          >
            {message}
          </Text>
        ))}
