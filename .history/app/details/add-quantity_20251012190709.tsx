...
export default function ManageStockScreen() {
  ...
  const [selections, setSelections] = useState<ProductSelection[]>([
    { productId: null, quantity: "", action: "add", unit: "unit" },
  ]);
      ...
      Alert.alert(
        ...
        [
          { 
            text: "OK", 
            onPress: () => {
              setSelections([{ productId: null, quantity: "", action: "add", unit: "unit" }]);
  ...
  const addAnotherSelection = useCallback(() => {
    ...
    const currentUnit = selections.length > 0 ? selections[0].unit : "unit";
    setSelections([...selections, { productId: null, quantity: "", action: currentAction, unit: currentUnit }]);
  }, [selections]);
          ...
          {item.productId && item.productId !== "add_new" && (
            <View style={styles.unitContainer}>
              <Dropdown
                style={[
                  styles.unitDropdown,
                  colorScheme === "dark" && styles.unitDropdownDark,
                  isSelected &&
                    (colorScheme === "dark"
                      ? styles.unitDropdownSelectedDark
                      : styles.unitDropdownSelectedLight),
                ]}
                placeholderStyle={[styles.unitPlaceholderStyle, colorScheme === "dark" && styles.unitPlaceholderStyleDark]}
                selectedTextStyle={[
                  styles.unitSelectedTextStyle,
                  colorScheme === "dark" && styles.unitSelectedTextStyleDark,
                ]}
                data={unitOptions}
                labelField="label"
                valueField="value"
                placeholder="Unit"
                value={item.unit}
                onChange={(selectedItem) => updateSelection(index, "unit", selectedItem.value)}
              />
            </View>
          )}
        </View>
    ...
    <View>
      ...
      {(duplicateProducts.length > 0 || invalidQuantities.length > 0 || (hasRemoveActions && !isRecipientValid)) && (
        <View style={[styles.validationSummary, colorScheme === "dark" && styles.validationSummaryDark]}>
          ...
          {/* Render validation items without mapping to avoid TypeScript errors */}
          <View>
           
            {hasRemoveActions && !isRecipientValid && (
              <ValidationItem
                message="• Select who is taking the products from stock"
              />
            )}
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  unitDropdownSelectedLight: { 
    borderColor: "#3b82f6", 
  },
  unitDropdownSelectedDark: { 
    borderColor: "#60a5fa", 
  },
  unitPlaceholderStyle: { 
    color: "#64748b", 
    fontSize: 12,
    textAlign: "center",
  },
  unitPlaceholderStyleDark: { 
    color: "#94a3b8" 
  },
  unitSelectedTextStyle: { 
    color: "#1e293b", 
    fontSize: 12,
    textAlign: "center",
    fontWeight: "600",
  },
  unitSelectedTextStyleDark: { 
    color: "#f1f5f9" 
  },
