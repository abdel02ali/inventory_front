...
export default function CreateCategoryScreen() {
  ...
  const predefinedCategoryList = [
    ...
    { name: 'Herbs & Spices', color: '#10b981' },
    { name: 'Grains & Pasta', color: '#d97706' },
    { name: 'Oils & Vinegars', color: '#f97316' },
    { name: 'Canned Goods', color: '#6b7280' },
    { name: 'Bakery', color: '#d4a574' },
    { name: 'Beverages', color: '#3b82f6' },
    { name: 'Cleaning Supplies', color: '#6366f1' },
    { name: 'Paper Goods', color: '#8b5cf6' },
    { name: 'Utensils', color: '#a855f7' },
    { name: 'Equipment', color: '#ec4899' },
    { name: 'Frozen Foods', color: '#06b6d4' },
    { name: 'Condiments', color: '#ef4444' },
    { name: 'Spices', color: '#f43f5e' },
    { name: 'Baking Supplies', color: '#f472b6' },
    { name: 'Fresh Herbs', color: '#84cc16' }
  ];
  ...
  const getCategoryColor = (categoryName: string, type: 'predefined' | 'custom') => {
    if (type === 'predefined') {
      const predefined = predefinedCategoryList.find(cat => cat.name === categoryName);
  ...
  const loadCategories = async () => {
    try {
      ...
      // If no predefined categories in DB, create them from our list
      if (predefined.length === 0) {
        const predefinedWithData = predefinedCategoryList.map(cat => ({
          _id: `predefined-${cat.name.toLowerCase().replace(/\s+/g, '-')}`,
          name: cat.name,
          type: 'predefined' as const,
          productCount: 0
        }));
        setPredefinedCategories(predefinedWithData);
      } else {
  ...
  const renderCategoryItem = ({ item }: { item: Category }) => {
    const categoryColor = getCategoryColor(item.name, item.type);
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.categoriesSection}>
        ...
        <Text style={styles.sectionDescription}>
          These categories are available by default and cannot be modified
        </Text>
