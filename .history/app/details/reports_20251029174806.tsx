...
// Utility function to safely get chart data
...
const StockLevelsSection: React.FC<{ stockLevels: ReportSummary['stockLevels'] }> = ({ stockLevels }) => {
  ...
  const barChartData = getChartData(
    stockStatusData.map(item => item.name),
    stockStatusData.map(item => item.count)
  );
...
const UsageAnalyticsSection: React.FC<{ usageAnalytics: ReportSummary['usageAnalytics'] }> = ({
  ...
  const usageByCategoryData = getChartData(usageByCategoryLabels, usageByCategoryValues);
...
const MovementStatisticsSection: React.FC<{ movementStats: ReportSummary['movementStatistics'] }> = ({
  ...
  const movementData = getChartData(movementLabels, movementValues);
    ...
    <View style={styles.section}>
      ...
      <View style={styles.chartContainer}>
        ...
        <BarChart
          ...
          width={screenWidth - 48}
          height={220}
          chartConfig={createChartConfig(movementColors)}
          style={styles.chart}
          yAxisLabel=""
          yAxisSuffix=""
          showValuesOnTopOfBars={true}
...
const DepartmentUsageSection: React.FC<{ departmentUsage: ReportSummary['departmentUsage'] }> = ({
  ...
  const barChartData = getChartData(departmentLabels, departmentValues);
        ...
        <View>
          <View style={styles.chartContainer}>
            ...
            <BarChart
              ...
              height={220}
              chartConfig={createChartConfig(departmentColors)}
              style={styles.chart}
              yAxisLabel=""
              yAxisSuffix=""
              showValuesOnTopOfBars={true}
...
const TopProductsSection: React.FC<{ usageAnalytics: ReportSummary['usageAnalytics'] }> = ({
  ...
  const productData = getChartData(productLabels, productValues);
        ...
        <View>
          <View style={styles.chartContainer}>
            ...
            <BarChart
              ...
              height={220}
              chartConfig={createChartConfig(productColors)}
