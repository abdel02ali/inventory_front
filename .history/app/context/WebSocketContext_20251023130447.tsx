...
export const WebSocketProvider: React.FC<WebSocketProviderProps> = ({ children }) => {
  ...
  const reconnectTimeout = useRef<number | null>(null); // Changed to number for React Native
  ...
  const attemptReconnect = (): void => {
    ...
    if (reconnectAttempts < maxAttempts) {
      ...
      reconnectTimeout.current = setTimeout(() => {
        ...
        connect();
      }, delay) as unknown as number; // Cast to number for React Native
    } else {
  ...
  const handleIncomingMessage = (message: WebSocketMessage): void => {
    switch (message.type) {
      ...
      case 'LOW_STOCK_ALERT':
        ...
        Alert.alert(
          'Low Stock Alert',
          `${message.data.name} is running low (${message.data.quantity} left)`
        );
        break;
