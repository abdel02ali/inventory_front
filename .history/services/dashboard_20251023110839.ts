...
export const getOutOfStockProducts = async () => {
  try {
    ...
    const response = await axios.get(`${API_URL}/api/products/out-of-stock`);
...
export const getLowStockProducts = async (threshold = 10) => {
  try {
    ...
    const response = await axios.get(`${API_URL}/api/products/low-stock?threshold=${threshold}`);
...
export const getRecentMovements = async (limit = 5) => {
  try {
    ...
    const response = await axios.get(`${API_URL}/api/movements/recent?limit=${limit}`);
