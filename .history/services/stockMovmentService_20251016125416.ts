...
const API_URL = "http://192.168.100.32:3000";
...
export const stockMovementService = {
  ...
  async getMovements(filters: {
    ...
    try {
      ...
      console.log('📡 Fetching movements from:', `${API_URL}/api/movements/history?${params}`);
      
      const response = await axios.get(`${API_URL}/api/movements/history?${params}`);
  ...
  async getDepartmentStats(department: Department) {
    try {
      const response = await axios.get(`${API_URL}/api/movements/departments/${department}`);
