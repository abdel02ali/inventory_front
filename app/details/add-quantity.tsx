...
export default function ManageStockScreen() {
  ...
  const handleManageStock = async () => {
    ...
    try {
      ...
      console.log('📤 Managing product stock:', { productsToAdd, productsToRemove });
  ...
  ), [colorScheme, refreshing, globalAction, setGlobalAction]);
  ...
  placeholder="Qty"
  placeholderTextColor={colorScheme === "dark" ? "#94a3b8" : "#64748b"} // Changed light mode color
...
/>
  ...
  const renderFooter = useCallback(() => (
    <View>
      ...
      {/* Validation Summary */}
      {(duplicateProducts.length > 0 || invalidQuantities.length > 0) && (
      ...
      <TouchableOpacity
        style={[
          styles.button, 
          (loading || refreshing || !hasValidSelections || duplicateProducts.length > 0 || invalidQuantities.length > 0) && styles.buttonDisabled,
          colorScheme === "dark" && styles.buttonDark
        ]}
        onPress={handleManageStock}
        disabled={loading || refreshing || hasUnhandledAddNew || !hasValidSelections || duplicateProducts.length > 0 || invalidQuantities.length > 0}
      >
  ...
  ), [colorScheme, refreshing, duplicateProducts, invalidQuantities, totalAdd, totalRemove, productsAdd, productsRemove, loading, hasUnhandledAddNew, hasValidSelections, addAnotherSelection, getProductName, handleManageStock]);
